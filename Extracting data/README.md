...
### get string data (extract_first() /get)
```python
>>> response.css('div.quote span.text').extract_first()
'<span class="text" itemprop="text">“The world as we have created it is a process of our thinking. It cannot be changed without changing our thinking.”</span>'
```

也可以使用,但如果extract 回傳None 會報錯

```python
>>> response.css('div.quote span.text').extract()[0]
'<span class="text" itemprop="text">“The world as we have created it is a process of our thinking. It cannot be changed without changing our thinking.”</span>'
```

### how to get text /attr data
...
>>> response.css('div.quote div.tags a::attr(href)').extract_first()
'/tag/change/page/1/'
```
...
對照css response.css('div.quote span.text')
